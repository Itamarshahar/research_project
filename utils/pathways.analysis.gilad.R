...
EnrichmentAnalysis <- function(de, formula="id~cluster", fun=c("enrichKEGG","enrichPathway","enrichGO:BP","enrichGO:MF","enrichGO:CC"), species="Homo sapiens", universe=NULL) {
  ...
  pathways <- list(`enrichKEGG` = function() compareCluster(f, data = de, fun="enrichKEGG", organism="mmu", universe=universe),
                   ...
                   `enrichGO:BP` = function() clusterProfiler::simplify(compareCluster(f, data = de, fun="enrichGO", OrgDb="org.Mm.eg.db", ont="BP", universe=universe)),
                   `enrichGO:MF` = function() clusterProfiler::simplify(compareCluster(f, data = de, fun="enrichGO", OrgDb="org.Mm.eg.db", ont="MF", universe=universe)),
                   `enrichGO:CC` = function() clusterProfiler::simplify(compareCluster(f, data = de, fun="enrichGO", OrgDb="org.Mm.eg.db", ont="CC", universe=universe)))
